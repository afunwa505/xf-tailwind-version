    ...
    if(!empty($_FILES["file"]["name"]) && $user_id){ 
        ...
        if(in_array(strtolower($fileType), $allowTypes)){ 
            if(move_uploaded_file($_FILES["file"]["tmp_name"], $targetFilePath)){ 
                $sql = "INSERT INTO audio (id, audio, user_id) VALUES ('".$id."','".$fileName."','".$user_id."')";
                $result = mysqli_query($conn,$sql);

                if($result){ 
                    header("Location: audio.php?upload=success");
                    exit();
                }else{ 
...
if(isset($_GET['id'])){
    ...
    header("Location: audio.php?deleted=1");
    exit();
}
?>
<!DOCTYPE html>
  ...
  <main class="p-6">
   <div class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-6">
      ...
      if(isset($_SESSION['id'])){
        $sql = "SELECT * FROM audio WHERE user_id='".$_SESSION['id']."' ORDER BY id DESC";
        $query = mysqli_query($conn,$sql);
        while($row = mysqli_fetch_object($query)){
          $audioURL = 'db-audios/'.htmlspecialchars($row->audio);
          echo "
          <div class='bg-white dark:bg-gray-700 p-4 rounded-lg shadow flex flex-col gap-3'>
            ...
            <div class='flex justify-between'>
              <a href='".$audioURL."' download class='bg-blue-500 hover:bg-blue-600 text-white text-xs px-3 py-1 rounded transition'>Download</a>
              <a href='audio.php?id=".$row->id."' onclick='return confirmDelete()' class='bg-red-500 hover:bg-red-600 text-white text-xs px-3 py-1 rounded transition'>Delete</a>
            </div>
          </div>";
        }
      }
